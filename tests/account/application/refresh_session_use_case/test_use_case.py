import uuid

import pytest
from freezegun import freeze_time

from src.account.application.refresh_session_use_case.dto import RefreshSessionDTO
from src.account.application.refresh_session_use_case.service import (
    RefreshSessionService,
)
from src.account.application.refresh_session_use_case.use_case import (
    RefreshSessionUseCase,
)
from src.account.application.session_claims_service import SessionClaimsService
from src.account.domain.entities.user_entity import UserEntity
from src.account.domain.entities.user_tenant_entity import UserTenantEntity
from src.account.domain.exceptions import InvalidSessionError
from src.account.shared.account_enums import UserTenantRoleEnum
from src.shared.infrastructure.auth import SessionClaims, TokenService


@pytest.fixture
def token_service() -> TokenService:
    return TokenService(secret_key="test-secret-key", exp_minutes=15)


def _make_use_case(
    fake_user_repository,
    fake_user_tenant_repository,
    fake_permission_version_repository,
    token_service,
) -> RefreshSessionUseCase:
    claims_service = SessionClaimsService(
        fake_user_tenant_repository, fake_permission_version_repository
    )
    return RefreshSessionUseCase(
        RefreshSessionService(fake_user_repository, claims_service, token_service),
        token_service,
    )


@pytest.mark.asyncio
async def test_refresh_of_a_stale_but_valid_session_reflects_the_current_role(
    fake_user_repository,
    fake_user_tenant_repository,
    fake_permission_version_repository,
    token_service,
):
    user = UserEntity(id=uuid.uuid4(), email="alice@example.com", is_superuser=False)
    tenant_id = uuid.uuid4()
    membership = UserTenantEntity(
        id=uuid.uuid4(),
        user_id=user.id,
        tenant_id=tenant_id,
        role=UserTenantRoleEnum.COLLABORATOR,
        is_active=True,
    )
    fake_user_repository.add_user(user, password="secret123")
    fake_user_tenant_repository.add_membership(membership)

    # Old token minted before a role change and a permission-version bump.
    old_claims = SessionClaims(
        user_id=user.id,
        is_staff=False,
        active_tenant_id=tenant_id,
        user_tenant_id=membership.id,
        role="COLLABORATOR",
        user_version=0,
        user_tenant_version=0,
    )
    old_token = token_service.encode(old_claims)
    fake_user_tenant_repository.add_membership(
        UserTenantEntity(
            id=membership.id,
            user_id=user.id,
            tenant_id=tenant_id,
            role=UserTenantRoleEnum.ADMIN,
            is_active=True,
        )
    )
    await fake_permission_version_repository.bump_user_tenant_version(membership.id)

    use_case = _make_use_case(
        fake_user_repository,
        fake_user_tenant_repository,
        fake_permission_version_repository,
        token_service,
    )
    result = await use_case.execute(
        RefreshSessionDTO(token=old_token, correlation_id="test-correlation-id")
    )

    assert result.role == "ADMIN"
    decoded = token_service.decode(result.token)
    assert decoded.user_tenant_version == 1


@pytest.mark.asyncio
async def test_refresh_rejected_for_an_expired_token(
    fake_user_repository,
    fake_user_tenant_repository,
    fake_permission_version_repository,
    token_service,
):
    user = UserEntity(id=uuid.uuid4(), email="alice@example.com", is_superuser=True)
    fake_user_repository.add_user(user, password="secret123")

    with freeze_time("2026-01-01 00:00:00"):
        claims = SessionClaims(user_id=user.id, is_staff=True)
        token = token_service.encode(claims)

    use_case = _make_use_case(
        fake_user_repository,
        fake_user_tenant_repository,
        fake_permission_version_repository,
        token_service,
    )

    with freeze_time("2026-01-01 01:00:00"):
        with pytest.raises(InvalidSessionError):
            await use_case.execute(
                RefreshSessionDTO(token=token, correlation_id="test-correlation-id")
            )


@pytest.mark.asyncio
async def test_refresh_rejected_for_a_tampered_token(
    fake_user_repository,
    fake_user_tenant_repository,
    fake_permission_version_repository,
    token_service,
):
    use_case = _make_use_case(
        fake_user_repository,
        fake_user_tenant_repository,
        fake_permission_version_repository,
        token_service,
    )

    with pytest.raises(InvalidSessionError):
        await use_case.execute(
            RefreshSessionDTO(
                token="not-a-real-token", correlation_id="test-correlation-id"
            )
        )
